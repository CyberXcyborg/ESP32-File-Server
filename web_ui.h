...
.header-right{display:flex;align-items:center;gap:10px;font-size:13px;color:var(--text2);flex-wrap:wrap}
...
.search-box input{border:none;background:transparent;padding:8px 0;outline:none;color:var(--text);font-size:13px;width:160px}
...
.file-list-header{display:grid;grid-template-columns:30px 1fr 100px 80px 120px;background:var(--primary);color:#fff;padding:10px 15px;font-weight:600;font-size:13px;cursor:pointer;user-select:none}
.file-list-header div:hover{opacity:.8}
.file-item{display:grid;grid-template-columns:30px 1fr 100px 80px 120px;padding:10px 15px;border-bottom:1px solid var(--border);align-items:center;font-size:13px;transition:background .15s}
...
.ctx-item{padding:10px 16px;cursor:pointer;font-size:13px;display:flex;align-items:center;gap:8px;transition:background .15s}
.ctx-item:hover{background:var(--bg)}
.ctx-item.danger{color:(--danger)}
...
/* Nav tabs */
.nav-tabs{display:flex;gap:4px;margin-bottom:12px;background:var(--card);border-radius:var(--radius);padding:4px;box-shadow:var(--shadow)}
.nav-tab{padding:8px 16px;border-radius:6px;cursor:pointer;font-size:13px;font-weight:500;color:var(--text2);transition:all .2s;border:none;background:transparent}
...
<div class="container">
  <header>
    <h1>📁 ESP32 File Server <small style="font-size:12px;color:var(--text2)">v2.1</small></h1>
  ...
  <div class="nav-tabs">
    <button class="nav-tab active" onclick="switchView('files')">📂 Files</button>
    <button class="nav-tab" onclick="switchView('trash')">🗑️ Trash</button>
    <button class="nav-tab admin-only" onclick="switchView('users')">👥 Users</button>
    <button class="nav-tab admin-only" onclick="switchView('log')">📋 Activity</button>
  </div>
  ...
  <div id="usersView" class="view">
    <div class="controls">
      <button class="btn" onclick="showUserModal()">+ Add User</button>
    </div>
    <div style="background:var(--card);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden">
      <table style="width:100%;border-collapse:collapse">
        <thead><tr style="background:var(--primary);color:#fff">
          <th style="padding:10px 15px;text-align:left;font-size:13px">Username</th>
          <th style="padding:10px 15px;text-align:left;font-size:13px">Role</th>
          <th style="padding:10px 15px;text-align:right;font-size:13px">Actions</th>
        </tr></thead>
  ...
  <div id="logView" class="view">
    ...
    <div style="background:var(--card);border-radius:var(--radius);box-shadow:var(--shadow);overflow:auto;max-height:70vh">
      <table class="log-table">
        <thead><tr><th>Time</th><th>User</th><th>Action</th><th>Path</th></tr></thead>
...
<div class="modal" id="shareModal">
  <div class="modal-content" style="max-width:500px">
    ...
    <div class="modal-body">
      <p style="font-size:13px;color:var(--text2);margin-bottom:12px">Anyone with this link can download the file without logging in.</p>
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){document.querySelectorAll('.modal').forEach(m=>m.style.display='none');hideCtxMenu();}});
});
...
function hideCtxMenu(){document.getElementById('ctxMenu').classList.remove('show');document.getElementById('ctxMenu').style.display='none';}
...
function uploadFiles(fileList,i){
  ...
  const file=fileList[i];const id='ul-'+file.name.replace(/[^a-zA-Z0-9]/g,'_');
...
function showRenameModal(path,name){
  document.getElementById('renamePath').value=path;document.getElementById('renameInput').value=name;
  openModal('renameModal');document.getElementById('renameInput').focus();document.getElementById('renameInput').select();
}
...
function shareFile(path){
  fetch('/api/share',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Authorization':'Bearer '+token},body:'path='+encodeURIComponent(path)})
    .then(r=>r.json())
    .then(data=>{document.getElementById('shareUrl').value=data.url;openModal('shareModal');})
  ...
  fetch('/api/trash',{headers:{'Authorization':'Bearer '+token}})
    ...
    .then(data=>{
      ...
      trashFiles.forEach(f=>{
        html+=`<div class="file-item"><div class="file-icon">${f.type==='dir'?'📁':'📄'}</div><div class="file-name">${f.name}</div><div class="file-size">${f.type==='dir'?'':formatSize(f.size)}</div><div class="file-actions" style="justify-content:flex-end">
          <span class="file-action" onclick="restoreItem('${f.path}')" title="Restore">♻️</span>
          <span class="file-action" onclick="permanentDelete('${f.path}','${f.name}')" title="Delete permanently">❌</span>
        </div></div>`;
      });
...
function permanentDelete(path,name){
  ...
  document.getElementById('confirmBtn').onclick=()=>{
    fetch('/api/empty-trash?path='+encodeURIComponent(path),{headers:{'Authorization':'Bearer '+token}})
      .then(r=>{if(r.ok){showToast('Deleted permanently','success');loadTrash();}else showToast('Failed','error');closeModal('confirmModal');})
      .catch(()=>{showToast('Failed','error');closeModal('confirmModal');});
  };
  openModal('confirmModal');
}
...
function loadUsers(){
  fetch('/api/users',{headers:{'Authorization':'Bearer '+token}})
    ...
    .then(data=>{
      ...
      let html='';
      users.forEach((u,i)=>{
        html+=`<tr><td style="padding:10px 15px;font-size:13px">${u.username}</td><td style="padding:10px 15px;font-size:13px">${u.userLevel}</td>
          <td style="padding:10px 15px;text-align:right">
            <button class="btn btn-sm" onclick="editUser(${i})">✏️</button>
            <button class="btn btn-sm btn-danger" onclick="deleteUser(${i})">🗑️</button>
          </td></tr>`;
      });
...
function editUser(i){
  ...
  document.getElementById('uName').value=users[i].username;
  document.getElementById('uPass').value='';
  document.getElementById('uRole').value=users[i].userLevel;
...
function saveUser(){
  ...
  const url=isEdit?'/api/users/'+n:'/api/users';
  fetch(url,{method:isEdit?'PUT':'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify(body)})
...
function deleteUser(i){
  ...
  fetch('/api/users/'+users[i].username,{method:'DELETE',headers:{'Authorization':'Bearer '+token}})
    .then(r=>{if(r.ok){loadUsers();showToast('Deleted','success');}else showToast('Failed','error');});
}
...
function loadLog(){
  fetch('/api/log',{headers:{'Authorization':'Bearer '+token}})
    ...
    .catch(()=>{document.getElementById('logTable').innerHTML='<tr><td colspan="4" style="padding:20px;text-align:center;color:var(--danger)">Error loading log</td></tr>';});
}
</script>
</body>
</html>
