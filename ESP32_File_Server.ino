...
/*
 * ESP32 File Server v2.1
 * (c) 2024 CyberXcyborg
 * 
 * Changelog v2.1:
 * - Activity log (tracks uploads, deletes, renames, downloads)
 * - File versioning (auto-backup on overwrite/rename)
 * - Share links (generate shareable URLs)
 * - PWA manifest (install as app)
 * - Context menu (right-click file operations)
 * - Improved trash UI
 * - File count in folder info
 */
...
void handleRename() {
  ...
  // Create version backup before rename
...
// ============== API: UPLOAD (with versioning on overwrite) ==============
...
void handleUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    else SD.remove(uploadPath); // remove partial
...
// Public share access (no auth needed)
    ...
    if (f) {
      ...
      for (int i = start; i < count; i++) {
        ...
        if (c1 > 0 && c2 > 0 && c3 > 0) {
          ...
          entry["time"] = l.substring(0, c1).toInt();
          entry["user"] = l.substring(c1 + 1, c2);
          entry["action"] = l.substring(c2 + 1, c3);
          entry["path"] = l.substring(c3 + 1);
        }
      }
    }
  }
...
void handleTrashPage() {
  ...
  // Serve the main UI — trash is now a view within the app
